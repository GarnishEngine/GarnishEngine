...
int main() {
    ...
    app.ecsManager.RegisterComponent<A>();
    auto e1 = app.ecsManager.CreateEntityWithComponents<A>(A{ -5 });
    auto e2 = app.ecsManager.CreateEntityWithComponents<A>(A{ 2 });

    app.ecsManager.AddSystem([](garnish::ECSManager* ecs){
        for (auto& entity : ecs->GetEntities<A>()) {
            auto a = ecs->GetComponent<A>(entity);
